/*
 * @Date: 2022-03-04 23:16:41
 * @Author: Mengsen Wang
 * @LastEditors: Mengsen Wang
 * @LastEditTime: 2022-03-09 14:48:23
 * @FilePath: /mengsen/mengsen/net/TcpConnection.cpp
 */

#include "mengsen/net/TcpConnection.h"

#include "mengsen/base/Logging.h"
#include "mengsen/base/WeakCallback.h"
...
void TcpConnection::send(const StringPiece& message) {
  if (state_ == kConnected) {
    if (loop_->isInLoopThread()) {
      ...
      // clear and definite sendInLoop() to fp, cause sendInLoop() has
      // overloaded
...
void TcpConnection::send(std::string&& message) {
  if (state_ == kConnected) {
    if (loop_->isInLoopThread()) {
      ...
      loop_->runInLoop([a{shared_from_this()}, message]() mutable {
        a->sendInLoop(std::forward<std::string>(message));
      });
    }
  }
}
...
void TcpConnection::send(Buffer&& buf) {
  if (state_ == kConnected) {
    if (loop_->isInLoopThread()) {
      ...
      loop_->runInLoop([a{shared_from_this()}, &buf]() {
        a->sendInLoop(std::forward<std::string>(buf.retrieveAllAsString()));
      });
    }
  }
}
...
void TcpConnection::sendInLoop(const void* data, size_t len) {
  ...
  // no force on writable event and output is total empty
...
void TcpConnection::shutdownAndForceCloseInLoop(double seconds) {
  ...
  // Prevent references from increasing
...
void TcpConnection::forceCloseWithDelay(double seconds) {
  if (state_ == kConnected || state_ == kDisconnecting) {
    setState(kDisconnecting);
    loop_->runInLoop(std::bind(&TcpConnection::forceCloseWithDelayInLoop,
                               shared_from_this(), seconds));
  }
}

void TcpConnection::forceCloseWithDelayInLoop(double seconds) {
  loop_->assertInLoopThread();
  loop_->runAfter(seconds, makeWeakCallback(shared_from_this(),
                                            &TcpConnection::forceCloseInLoop));
}
...
void TcpConnection::handleRead(Timestamp::Time_uint64_t receiveTime) {
  loop_->assertInLoopThread();
  int savedErrno = 0;
...
void TcpConnection::handleWrite() {
  ...
  if (channel_->isWriting()) {
    ...
    if (n > 0) {  // write successfully
      outputBuffer_.retrieve(n);
      if (outputBuffer_.readableBytes() == 0) {  // total write in outputBuffer_
        channel_->disableWriting();              // disable write event on force
